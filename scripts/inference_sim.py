...
from forward_model import old_model, old_score_likelihood, link_function
...
def main(args): 
    ...
    # S = sampling_function.reshape(256, 256).to(device) # Sampling function in padded Fourier space
    S = torch.tensor(np.load(args.sampling_function))
    ...
    observation = old_model(t = torch.zeros(1).to(device), 
                        x = ground_truth[0],
    ...
    for i in range(int(num_samples//batch_size)):
        if sampler.lower() == "euler":
            ...
            sampling_params = (predictor, None, None) 
            samples = euler_sampler(
                y = observation,
                sigma_y = sigma_y,
                forward_model = old_model, 
                score_model = score_model,
                score_likelihood = old_score_likelihood, 
                model_parameters = model_parameters,
        ...
        y_hat = vmap(lambda x: old_model(t = torch.zeros(1).to(device), 
                        x = x,
                        score_model = score_model, 
                        model_parameters = model_parameters), randomness = 'different')(samples) 
    ...
    if args.sanity_plot: 
        ...
        image_dir = os.path.join(path_experiment, f"sanity_{THIS_WORKER}.jpeg")
